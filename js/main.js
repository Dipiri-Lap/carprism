    ...
    track.innerHTML = heroItems.map((item, i) => `
      <div class="slide" role="group" aria-label="슬라이드 ${i + 1}" data-href="articles/${item.slug}.html" tabindex="0">
        ...
        <div class="slide-content">
          ...
          <p class="slide-meta">편집팀 &nbsp;·&nbsp; ${item.date.replace(/-/g, '.')}${item.badge === 'TEST DRIVE' ? ` &nbsp;·&nbsp; <span class="slide-source">원문: ${item.source}</span>` : ''}</p>
      ...
      <li>
        <a href="articles/${item.slug}.html" class="article-row reveal">
          ...
          <div class="row-info">
            ...
            <p class="row-meta">${item.date.replace(/-/g, '.')}${item.badge === 'TEST DRIVE' ? ` · 원문: ${item.source}` : ''}</p>
          </div>
        </a>
      </li>
    `).join('');
